...
const { usage, read: readOptions } =  getOptions({
  description: "Convert an XHTML, SVG, or XML file to the POJSO format supported by @fordi/create-element",
  validate: (options) => {
    if (!options.inFile) {
  ...
  o: {
    name: 'out',
    description: 'Output JS file (default: moduleName.ext in same dir as input file',
...
const nsrMap = Object.keys(nsMap).reduce((o, k) => ({ ...o, [nsMap[k]]: k }), {});
...
const toArray = (element) => {
  ...
  const attrPairs = toArray(attributes).map(({ name, value }) => {
    if (name === 'xmlns') {
...
const toComponentSource = async ({ inFile, outFile, name, ext = 'js' }) => {
  ...
  const dom = parser.parseFromString(await readFile(inFile, 'utf-8'), 'text/xml');
  const code = toArray(dom.documentElement);
  let content = [
    ...[...usedNS].map((key) => `const ${key} = ${JSON.stringify(nsMap[key])};`),
    `const ${name} = () => (\n${indent(code)}\n);`,
    `export default ${name};`
  ];
